...
1. **출처 우선** — Notion 규칙 페이지(`프론트엔드 아키텍처`)와 EPIC DB가 단일 진실. 추측 전에 `mcp__notion__notion-fetch`로 해당 EPIC/스토리를 읽는다. 수용 기준(acceptance criteria)을 그대로 명세에 옮긴다.
   - **API 계약(경로·메서드·요청/응답 shape·에러코드)은 `frontend-feature/references/api-spec.md`를 따른다.** 전역 응답 봉투·에러코드·도메인 enum은 그 문서에 박혀 있고, 개별 엔드포인트는 거기 적힌 조회법으로 API 명세서 Notion DB를 fetch해 확인한다. 경로·필드를 추측하지 말고 명세 행을 인용한다.
     - **명세 DB에 필요한 엔드포인트가 없으면** 임의로 만들지 말고 명세에 `⚠️ 명세없음: <기능> — <METHOD path 추정>`으로 표시하고 리더에 보고한다(리더가 사용자에 확인 → Notion 추가 후 진행). `⚠️ 작명필요`와 동일한 에스컬레이션.
