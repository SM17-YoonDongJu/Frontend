...
2. **빌드 패턴 적용** — `component-build` 스킬의 App Router·라우트그룹·Server/Client 경계·Tailwind·상태표현 패턴을 따른다.
   ...
   - **길이값은 rem** — 폰트 크기·간격·너비 등 모든 길이값은 rem(16px=1rem), Tailwind 스케일 유틸 우선. **`[Npx]` 임의값 금지(1px 보더만 예외)**. 기존 레포에 px가 남아 있어도 따라 하지 말 것. 규칙·변환표는 design-tokens.md 「길이값」 절 + `figma-design-convert/references/figma-mapping.md §4`.
