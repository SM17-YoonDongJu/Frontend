    ...
    <section>
      ...
      <ul className="mt-3 flex flex-col gap-2">
        {DELETED_ITEMS.map((item) => (
          <li key={item} className="flex gap-2 text-[0.875rem] leading-relaxed text-ink-2">
            <span aria-hidden className="mt-2 size-1 shrink-0 rounded-full bg-ink-3" />
            <span>{item}</span>
          </li>
        ))}
      </ul>
      <p className="mt-4 rounded-card bg-terra-soft px-4 py-3 text-[0.8125rem] leading-relaxed text-terra">
