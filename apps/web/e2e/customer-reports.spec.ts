 ...
 * 흐름: 목록 진입(첫 페이지 5건) → 더보기(다음 페이지 누적 총 8건) → 카드 "리포트 보기" 상세 이동
...
test.describe("내 리포트 목록", () => {
  ...
  test("진입하면 첫 페이지 리포트 카드가 유형·사건번호·금액과 함께 보인다", async ({ page }) => {
    ...
    // 첫 페이지 5건(size 5) — 카드마다 "리포트 보기" 링크가 하나씩 있어 카드 수로 센다.
    await expect(page.getByRole("link", { name: /리포트 보기/ })).toHaveCount(5);
  ...
  test("더보기를 누르면 다음 페이지 리포트가 이어 붙어 총 8건이 된다", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /리포트 보기/ })).toHaveCount(8);
    await expect(moreButton).toHaveCount(0);
  });

  test("카드의 리포트 보기를 누르면 해당 리포트 상세로 이동한다", async ({ page }) => {
    await page.goto(PATH);
    await expect(page.getByRole("heading", { name: "내 리포트" })).toBeVisible();
    ...
    await expect(async () => {
      await firstCard.getByRole("link", { name: /리포트 보기/ }).click();
      await expect(page).toHaveURL(/\/customer\/report\/[0-9a-f-]+/);
    }).toPass({ timeout: 10000 });
  });
