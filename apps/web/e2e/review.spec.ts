 ...
 * 응답은 기본 MSW 핸들러가 제공(검수 대기 5건: 후유장해 2·교통사고 2·실손 1).
...
test("후유장해 칩을 선택하면 URL에 type이 붙고 해당 유형만 남는다", async ({ page }) => {
  ...
  // 필터 전: 교통사고 카드가 보인다
  await expect(page.getByText("다발성 늑골 골절 · 일실수입 과소 산정 의심")).toBeVisible();
  ...
  await expect(
    page.getByText("다발성 늑골 골절 · 일실수입 과소 산정 의심"),
  ).toHaveCount(0);
});
...
test("전체 칩으로 되돌리면 type 파라미터가 사라지고 모든 유형이 보인다", async ({ page }) => {
  ...
  await expect(page.getByText("다발성 늑골 골절 · 일실수입 과소 산정 의심")).toHaveCount(0);
...
test("카드의 검수 버튼을 누르면 검수 상세로 이동한다", async ({ page }) => {
  ...
  const card = page
    .getByRole("listitem")
    .filter({ hasText: "우측 슬관절 후방십자인대 파열 · 등급 재산정 쟁점" });
