 ...
 * 알림 설정 진입은 역할별로 갈린다 — 파트너는 모달 자동 오픈, 고객은 인라인 카드로 이동.
...
test.describe("PC 고객 헤더", () => {
  ...
  test("고객 헤더에서도 팝오버가 열리고 알림 설정은 마이페이지 카드로 이동한다", async ({ page }) => {
    ...
    await dialog.getByRole("button", { name: "알림 설정" }).click();
    await expect(page).toHaveURL(/\/customer\/mypage/);

    // 인라인 카드로 이동 — 카드가 보이고, 진입 후 panel 파라미터는 제거된다
    const card = page.getByRole("heading", { name: "알림 설정", exact: true });
    await expect(card).toBeInViewport({ timeout: 15000 });
    await expect(page).toHaveURL(/\/customer\/mypage$/);
  });
});
