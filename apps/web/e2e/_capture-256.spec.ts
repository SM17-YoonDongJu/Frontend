...
/** PR 스크린샷 캡처 헬퍼(#256). 테스트 아님 — CI 제외(testIgnore). */

const DIR = "../../.pr-assets/issue-256";
...
test("01 본인 확인 스텝 — 이름·지역 입력", async ({ page }) => {
  await page.setViewportSize(DESKTOP);
  await goToIdentity(page);
  await fillIdentity(page);
  await page.screenshot({ path: `${DIR}/01-identity-desktop.png` });
});

test("02 완료 화면 — 입력한 이름 노출", async ({ page }) => {
  await page.setViewportSize(DESKTOP);
  await goToIdentity(page);
  await fillIdentity(page);
  await page.getByRole("button", { name: "다음" }).click();
  await expect(page.getByRole("heading", { name: "가입이 완료됐어요" })).toBeVisible();
  await page.screenshot({ path: `${DIR}/02-complete-desktop.png` });
});

test("03 완료 화면 모바일", async ({ page }) => {
  await page.setViewportSize(MOBILE);
  await goToIdentity(page);
  await fillIdentity(page);
  await page.getByRole("button", { name: "다음" }).click();
  await expect(page.getByRole("heading", { name: "가입이 완료됐어요" })).toBeVisible();
  await page.screenshot({ path: `${DIR}/03-complete-mobile.png` });
});
