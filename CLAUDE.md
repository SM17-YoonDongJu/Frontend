...
| 2026-07-06 | ESLint→oxlint 전환 + CI 파이프라인 구축 (#58) | apps/web/.oxlintrc.json, package.json(lint), .githooks/pre-commit, packages/config(eslint 제거), playwright.config.ts, .github/workflows/ci.yml, code-conventions·fe-integration-qa 문서 | lint를 oxlint 단일화(플러그인 6종+기존 커스텀 룰 이식, scripts는 no-console 예외), ESLint 의존성·설정 전체 제거. CI(typecheck/lint/build/E2E 병렬, PR+main/dev push, Playwright 브라우저 캐시). E2E는 production 빌드로 검증 — MSW 기동을 NEXT_PUBLIC_API_MOCKING=enabled 플래그로도 허용. 브라우저 매트릭스 chromium/mobile-chrome/mobile-safari(webkit·msedge 제거), _capture-* 스펙 CI 제외 |
...
| 2026-07-09 | 이슈 작성·API명세 초안 스킬 추가 (#91) | skills/issue-craft, skills/api-spec-notion | 세션 79개 전수조사에서 반복 최다·스킬 부재로 식별: GitHub 이슈(초안 22개·재작업 질책 2회)와 Notion API명세 초안(Notion 툴 ~500회)을 팀 양식 스킬로 박음. issue-craft=두 갈래 양식(기능 `## 설명/작업 목록/완성 조건` · 비기능 `## 배경/작업 항목/완료 조건`)+제목 대괄호 태그+라벨(기능/인프라 `FE,Feature`·리팩터 `FE,Refactor`)+`--body-file` mojibake 방지+생성 전 동종 이슈 gh view 게이트+실제 예시 3종. api-spec-notion=명세 없는 엔드포인트 초안(봉투·에러코드 enum·필드 미러링, 추측 경로 금지)→백엔드 확정 후 Notion 동기화 |
| 2026-07-27 | 금액 표기 규칙 추가 | code-conventions | 전체 감사에서 원/만원 단위 혼재 검출(#190) — 입력 퍼널은 원 전액, 리포트·제안은 만원 요약에 변환 함수 4곳 복붙. 데이터=원 정수·표시 기본=만원 요약·입력/확인=원 전액으로 정책 확정, 포맷은 shared/lib 공용 유틸로만(인라인 `/10_000`·로케일 없는 toLocaleString 금지) |
| 2026-08-06 | 1파일 1컴포넌트 규칙 정정·승격 + 로직 분리 절 신설 (#250) | code-conventions(파일 구조 규칙·비즈니스 로직↔프레젠테이션 분리 절), component-build(한 줄 요약+포인터), .oxlintrc.json(react/no-multi-comp), .claude/hooks/one-component-lint.sh, .claude/settings.json | 실측 위반 65건/51파일인데 기존 문구가 "컴포넌트 하나만 **export**"라 로컬 프리미티브·폴백 동거를 합법화(다중 export는 4건뿐), 규칙이 "네이밍" 아래 묻혀 있고 component-build엔 언급 0건, 기계 강제도 없었음. 문구를 "컴포넌트 선언 하나만"으로 고쳐 파일 구조 규칙으로 승격하고, 흩어져 있던 로직·프레젠테이션 분리 지침을 층위표(_api/_hooks/_model/.tsx)로 통합. 백필 완료 후 oxlint 룰(stories 예외)+PostToolUse 훅으로 강제 |
| 2026-07-27 | 백엔드 요청 DB 카드 절차 스킬 분리 (#191) | skills/backend-request-notion(신설), skills/api-spec-notion(§5 이관) | api-spec-notion에 §5로 얹었던 "백엔드 요청(API·ERD) DB 카드 작성" 절차가 메모리에만 있던 규칙(본문 3단 구조·parent 명시·존댓말)을 실행 시 누락시키는 문제를 §5 신설로 우선 해결했으나, 대상 DB·트리거 문구가 명세 초안 작성과 달라 관심사 분리 판단 → 별도 스킬로 독립. api-spec-notion=명세 없는 엔드포인트 초안 작성(§0~4)만 유지, backend-request-notion=API변경·ERD·정책·버그 요청 카드(프로퍼티+본문 구조·존댓말·parent 명시·같은 대상 append) 전담. PR 점검 중 "결정 필요" 섹션이 마크다운 `>` 인용문으로 작성돼 있어 Notion enhanced markdown 스펙 대조 후 실제 콜아웃 블록 문법(`<callout icon="❓">`)으로 정정 |
