...
| 2026-07-27 | 백엔드 요청 DB 카드 절차 스킬 분리 (#191) | skills/backend-request-notion(신설), skills/api-spec-notion(§5 이관) | api-spec-notion에 §5로 얹었던 "백엔드 요청(API·ERD) DB 카드 작성" 절차가 메모리에만 있던 규칙(본문 3단 구조·parent 명시·존댓말)을 실행 시 누락시키는 문제를 §5 신설로 우선 해결했으나, 대상 DB·트리거 문구가 명세 초안 작성과 달라 관심사 분리 판단 → 별도 스킬로 독립. api-spec-notion=명세 없는 엔드포인트 초안 작성(§0~4)만 유지, backend-request-notion=API변경·ERD·정책·버그 요청 카드(프로퍼티+본문 구조·존댓말·parent 명시·같은 대상 append) 전담 |
